                             ...
                             int rc)
{
    ...
    /* find the error that was not ERROR_REMUS_DEVOPS_NO_MATCH */
    for (i = 0; i < rds->num_devices; i++) {
        dev = rds->dev[i];

        if (!dev->aodev.rc || dev->aodev.rc == ERROR_REMUS_DEVOPS_NO_MATCH)
            continue;
    ...
    /* if the error is still ERROR_REMUS_DEVOPS_NO_MATCH, begin next iter */
    if (rc == ERROR_REMUS_DEVOPS_NO_MATCH) {
        remus_devices_setup(egc, rds);
        return;
    }
