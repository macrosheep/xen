                                ...
                                libxl__remus_devices_state *rds)
{
    ...
    for (i = 0; i < rds->num_devices; i++) {
        ...
        do {
            ...
            if (!dev->ops) {
                rc = ERROR_REMUS_DEVICE_NOT_SUPPORT;
                goto out;
            }
                             ...
                             int rc)
{
    ...
    /* find the error that was not ERROR_REMUS_DEVOPS_NOT_MATCH */
    for (i = 0; i < rds->num_devices; i++) {
        dev = rds->dev[i];

        if (!dev->aodev.rc || dev->aodev.rc == ERROR_REMUS_DEVOPS_NOT_MATCH)
            continue;
    ...
    /* if the error is still ERROR_REMUS_DEVOPS_NOT_MATCH, begin next iter */
    if (rc == ERROR_REMUS_DEVOPS_NOT_MATCH) {
        remus_devices_setup(egc, rds);
        return;
    }
